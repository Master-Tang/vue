...
export default {
  ...
  created() {
    ...
    //房产信息
    $.propertyList({ collateralId: this.$route.query.claimsNumber,busType:"01" }).then(
      res => {
        if (res.success) {
    ...
    //土地信息
    $.landList({ collateralId: this.$route.query.claimsNumber,busType:"01" }).then(res => {
      if (res.success) {
        this.list2 = res.data;
    ...
    //其他资产信息
    $.otherList({ collateralId: this.$route.query.claimsNumber,busType:"01" }).then(res => {
      if (res.success) {
        this.list3 = res.data;
      ...
      this.$confirm("此操作将删除该伙伴, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          $.otherFake({ otherId: id }).then(response => {
            ...
            $.otherList({ claimsNumber: this.$route.query.claimsNumber }).then(res => {
      if (res.success) {
        this.list3 = res.data;
        for (var i = 1; i <= this.list3.length; i++) {
          this.$set(this.list3[i - 1], "a", i);
