      ...
      <div v-for="(item,index) in exitplanList" :key="index">
        <div class="my-padding">
          ...
          <el-row style="padding-bottom: 1rem">
            ...
            <el-col :span="5">
              <el-input size="small" v-model="item.money" @change="changeMoney(item.money,item.exitId)"></el-input>
            </el-col>
          ...
          <el-table :data="item.scheme" border style="width: 100% ;padding-top: 2rem;">
            ...
            <el-table-column align="center" label="回收金额" width="150rem">{{item.money}}</el-table-column>
          </el-table>
        </div>
      </div>
    </div>
        ...
        <el-col :span="22">
          <div v-for="(item, index) in form.scheme" :key="index">
            <div style="width:110%">
              ...
              <el-row style="padding-bottom: 1rem">
                <el-col :span="5">
                  <el-select v-model="item.guaranteeSolute" placeholder="请选择保底回收方案" style="width:100%">
                    <el-option
                ...
                <el-col :span="2">
                  <el-button icon="el-icon-delete" circle @click="deleteItem(item, index)"
                  ></el-button>
                </el-col>
              </el-row>
            </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchData() {
        $.busExitsList({claimsNumber: this.$route.query.claimsNumber}).then(res => {
